...
describe("users collection", () => {
	...
	it("a user can edit their own bio/displayName", async () => {
		await assertSucceeds(updateDoc(doc(aliceDb(), "users", ALICE), { bio: "hello", displayName: "Al" }));
	});
...
describe("posts collection", () => {
	...
	it("clients cannot write posts directly (functions only)", async () => {
		await assertFails(setDoc(doc(aliceDb(), "posts", "p3"), { authorId: ALICE, status: "active", score: 100 }));
	});
});
