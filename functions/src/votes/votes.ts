 ...
 */
async function applyVote(uid: string, targetType: TargetType, targetId: string, value: 1 | -1 | 0): Promise<{ score: number; value: 1 | -1 | 0 }> {
	...
	return db.runTransaction(async (tx) => {
		...
		if (targetType === "post") {
			const createdAtMs = (target.createdAt as { toMillis?: () => number } | undefined)?.toMillis?.() ?? Date.now();
