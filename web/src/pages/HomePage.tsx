		...
		<div className="space-y-8">
			<section className="rounded-2xl border border-line bg-surface p-6 shadow-soft sm:p-8">
				...
				<div className="mt-5 flex flex-wrap gap-3">
					<Link to="/login" className="rounded-full bg-coral px-5 py-2.5 font-medium text-white transition-transform hover:scale-[1.02]">
						Join the community
					</Link>
					<Link
						to="/c/general"
				...
				<div className="grid gap-3 sm:grid-cols-2">
					{COMMUNITIES.map((c) => (
						<Link
							...
							<div className="font-medium text-ink group-hover:text-coral">{c.name}</div>
