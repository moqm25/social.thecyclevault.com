...
export const reserveUsername = callable<
  { username: string },
  { available: boolean; reserved?: boolean }
>('reserveUsername');
...
export const updatePost = callable<
  { postId: string; title?: string; body?: string; tags?: string[] },
  { ok: true }
>('updatePost');
...
export const updateComment = callable<{ commentId: string; body: string }, { ok: true }>(
  'updateComment',
);

export const deleteCommentSoft = callable<{ commentId: string }, { ok: true }>(
  'deleteCommentSoft',
);

// ---- voting ----
export const voteOnPost = callable<
  { postId: string; value: VoteValue | 0 },
  { score: number; value: VoteValue | 0 }
>('voteOnPost');

export const voteOnComment = callable<
  { commentId: string; value: VoteValue | 0 },
  { score: number; value: VoteValue | 0 }
>('voteOnComment');
...
// ---- notifications ----
export const markNotificationRead = callable<
  { notificationId?: string; all?: boolean },
  { ok: true }
